...
class PostController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        */
        ...
        try {
            $data['is_active'] = true;
            // como os 'posts' devem pertencem a um usuário a linha abaixo força que os 'posts' pertençam
            // ao usuario 1
            // $user = User::find(1);
            // utilizando o 'middleware' de autenticação para obter as informações do usuário autenticado
            $user = auth()->user();
            // realizando o insert na tabela de posts
            $post = $user->posts()->create($data);
            // capturando as categorias selecionadas para cada post e salvando na tabela associativa
    ...
    public function update(Post $post, Request $request) {
        ...
        */
        try {
            // atualizando os dados na tabela de posts
            $post->update($data);
